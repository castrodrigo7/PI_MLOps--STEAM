...
def userdata(user_id: str):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error en la función userdata: {str(e)}")
...
def UserForGenre(genero: str):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error en la función UserForGenre: {str(e)}")
...
def best_developer_year(year: int):
    try:
        ...
        if year not in df['year'].values:
            raise HTTPException(status_code=404, detail=f"No hay datos disponibles para el año {year}")
        ...
        raise HTTPException(status_code=500, detail=f"Error en la función best_developer_year: {str(e)}")
